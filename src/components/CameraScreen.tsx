...
export const CameraScreen: React.FC<CameraScreenProps> = ({
  ...
  // one physical camera.
  const primary: CameraDevice | undefined = useCameraDevice(position);
  const fallbackPosition: CameraPosition = position === 'front' ? 'back' : 'front';
  const secondary: CameraDevice | undefined = useCameraDevice(fallbackPosition);
  const device = primary ?? secondary;
  const effectivePosition: CameraPosition = primary
    ? position
    : secondary
      ? fallbackPosition
      : position;
  ...
  if (!device) {
    return (
      <View style={[styles.root, styles.center]}>
        ...
        <Text style={[styles.muted, styles.subtle]}>
          {resolvedUI.strings.cameraDeviceUnavailable}
        </Text>
        <Pressable
          onPress={onCancel}
          hitSlop={12}
