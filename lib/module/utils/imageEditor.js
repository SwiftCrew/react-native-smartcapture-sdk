 ...
 */
export function getImageSize(uri, timeoutMs = GET_SIZE_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    let settled = false;
    const timer = setTimeout(() => {
      if (settled) return;
      settled = true;
      reject(new SmartCaptureError('image_load_timeout', `Loading image dimensions timed out after ${timeoutMs}ms.`));
    }, timeoutMs);
    Image.getSize(uri, (width, height) => {
      ...
      if (!isFiniteSize(width) || !isFiniteSize(height)) {
        reject(new SmartCaptureError('image_load_failed', `Image reported invalid size (${width}x${height}).`));
        return;
      }
      resolve({
        width,
      ...
      settled = true;
      clearTimeout(timer);
      reject(toSmartCaptureError(err, 'image_load_failed', 'Could not load image.'));
    });
  });
}
...
export async function cropImage(args) {
  ...
  let cropResult;
  try {
    cropResult = includeBase64 ? await ImageEditor.cropImage(uri, {
      ...baseArgs,
      includeBase64: true
  ...
  if (!outUri || typeof outUri !== 'string') {
    throw new SmartCaptureError('crop_failed', 'Image editor returned no URI.', cropResult);
  }
